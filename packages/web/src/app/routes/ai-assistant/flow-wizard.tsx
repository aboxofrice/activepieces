...
const WIZARD_STEPS = [
  ...
  },
  {
    id: 'destination',
    title: 'Destination',
    description: 'Where should the data go?',
  },
...
const canBeSource = (piece: PieceMetadataModelSummary): boolean => {
  return (
    piece.triggers > 0 ||
    piece.name === '@activepieces/piece-schedule' ||
...
export const FlowWizard = ({
  ...
  const { data: sourcePieceDetails, isLoading: isLoadingSourceDetails } =
    useQuery({
  ...
  const triggers = useMemo(() => {
    ...
    return Object.entries(sourcePieceDetails.triggers).map(
      ([key, trigger]) => ({
        value: key,
  ...
  const importMutation = useMutation({
    mutationFn: async (flowJson: string) => {
      ...
      if (!flowData.trigger) {
        throw new Error(
          'Flow must have a trigger. The AI may have generated an incomplete flow.',
        );
      }
      ...
      while (currentAction) {
        ...
        if (
          currentAction.type === 'PIECE' &&
          !currentAction.settings?.pieceName
        ) {
          throw new Error(
            `Action "${currentAction.name}" must have a pieceName in settings.`,
          );
        }
        ...
        if (currentAction.type === 'BRANCH') {
          ...
          if (!currentAction.settings?.conditions) {
            throw new Error(
              `Branch "${currentAction.name}" must have conditions.`,
            );
          }
      ...
      const displayName =
        ...
        parsedFlow.name ||
        `${wizardData.sourceSystemName} to ${wizardData.destinationSystemName}`;
  ...
  const generateMutation = useMutation({
    ...
    onError: (error: unknown) => {
      ...
      if (
        ...
        setErrorMessage(
          t(
            'AI provider not configured. Please configure an AI provider in Platform Settings → AI Providers.',
          ),
        );
      } else if (
        ...
        setErrorMessage(
          t(
            'AI provider quota exceeded or out of credits. Please add credits to your provider account or switch to a different provider.',
          ),
        );
      } else if (
        ...
        setErrorMessage(
          t(
            'Invalid API key for the selected AI provider. Please check your configuration.',
          ),
        );
      } else if (status === 500) {
        ...
        setErrorMessage(
          t('Server error: {message}', { message: shortMessage }),
        );
      } else {
  ...
  const handleGenerate = () => {
    const triggerLabel =
      triggers.find((tr) => tr.value === wizardData.triggerType)?.label ||
      wizardData.triggerType;
    const actionLabel =
      actions.find((a) => a.value === wizardData.actionType)?.label ||
      wizardData.actionType;

    // Extract trigger props for context
    const selectedTrigger =
      sourcePieceDetails?.triggers?.[wizardData.triggerType];
    // eslint-disable-next-line @typescript-eslint/no-explicit-any
    const triggerProps = selectedTrigger?.props
      ? // eslint-disable-next-line @typescript-eslint/no-explicit-any
        Object.entries(selectedTrigger.props).map(
          ([key, prop]: [string, any]) => ({
            name: key,
    ...
    const actionProps = selectedAction?.props
      ? // eslint-disable-next-line @typescript-eslint/no-explicit-any
        Object.entries(selectedAction.props).map(
          ([key, prop]: [string, any]) => ({
            name: key,
            displayName: prop.displayName,
            required: prop.required,
...
<source-piece name="${wizardData.sourceSystem}" version="${
      sourcePieceDetails?.version || '~0.5.0'
    }">
  <trigger name="${wizardData.triggerType}" displayName="${triggerLabel}">
    <props>${JSON.stringify(triggerProps)}</props>
  </trigger>
</source-piece>
<destination-piece name="${wizardData.destinationSystem}" version="${
      destPieceDetails?.version || '~0.5.0'
    }">
...
${
  wizardData.triggerDescription
    ? `**Details:** ${wizardData.triggerDescription}`
    : ''
}

**Destination:** ${wizardData.destinationSystemName} (${
      wizardData.destinationSystem
    })
**Action:** ${actionLabel} (${wizardData.actionType})
${
  wizardData.actionDescription
    ? `**Details:** ${wizardData.actionDescription}`
    : ''
}

${
  wizardData.additionalDetails
    ? `**Additional:** ${wizardData.additionalDetails}`
    : ''
}
          ...
          <div className="space-y-4">
                ...
                {triggers.map((trigger) => (
                  <button
                    ...
                    className={cn(
                      ...
                      wizardData.triggerType === trigger.value &&
            ...
            <div className="mt-4">
              ...
              <Textarea
                ...
                onChange={(e) =>
                  updateData('triggerDescription', e.target.value)
                }
                placeholder={t(
                  'e.g., Only for premium customers, specific record types, etc.',
                )}
                className="mt-2"
                rows={2}
              />
          ...
          <div className="space-y-4">
              ...
              <div className="max-h-[350px] space-y-2 overflow-auto">
                {actions.map((action) => (
                  <button
                    ...
                    className={cn(
                      'flex w-full items-start gap-3 rounded-lg border p-3 text-left transition-all hover:border-primary',
                      wizardData.actionType === action.value &&
                        'border-primary bg-primary/5',
                    )}
                  >
            ...
            <div className="mt-4">
              ...
              <Textarea
                ...
                onChange={(e) =>
                  updateData('actionDescription', e.target.value)
                }
                placeholder={t(
                  'e.g., Create as Contact record, set status to Active, etc.',
                )}
                className="mt-2"
                rows={2}
              />
          ...
          <div className="space-y-4">
            <div className="rounded-lg bg-muted p-4">
              ...
              <div className="space-y-2 text-sm">
                ...
                <div className="flex justify-between">
                  ...
                  <span className="font-medium">
                    {wizardData.destinationSystemName}
                  </span>
                </div>
                <div className="flex justify-between">
            ...
            <div>
              ...
              <Textarea
                ...
                placeholder={t(
                  'e.g., Field mappings, conditions, error handling preferences...',
                )}
                className="mt-2"
                rows={3}
              />
